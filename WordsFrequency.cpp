...
std::string ComputeSha256(const std::string& input)
{
    ...
    // 1. Открыть провайдер алгоритма SHA-256
    status = BCryptOpenAlgorithmProvider(&hAlgorithm, BCRYPT_SHA256_ALGORITHM, nullptr, 0);
    if (status != 0) {
        std::cerr << "BCryptOpenAlgorithmProvider failed: 0x" << std::hex << status << std::dec << std::endl;
        return "";
    }

    // 2. Получить размер объекта хэша
    status = BCryptGetProperty(hAlgorithm, BCRYPT_OBJECT_LENGTH, (PBYTE)&cbHashObject, sizeof(DWORD), &cbResult, 0);
    if (status != 0) {
        std::cerr << "BCryptGetProperty(OBJECT_LENGTH) failed: 0x" << std::hex << status << std::dec << std::endl;
        BCryptCloseAlgorithmProvider(hAlgorithm, 0);
        return "";
    }

    // 3. Получить длину хэша (для SHA-256 это 32 байта)
    status = BCryptGetProperty(hAlgorithm, BCRYPT_HASH_LENGTH, (PBYTE)&cbHash, sizeof(DWORD), &cbResult, 0);
    if (status != 0) {
        std::cerr << "BCryptGetProperty(HASH_LENGTH) failed: 0x" << std::hex << status << std::dec << std::endl;
        BCryptCloseAlgorithmProvider(hAlgorithm, 0);
        return "";
    }

    // 4. Выделить память под объект хэша и под результат
    ...
    // 5. Создать объект хэша (ручное управление памятью)
    status = BCryptCreateHash(hAlgorithm, &hHash, pbHashObject, cbHashObject, nullptr, 0, 0);
    if (status != 0) {
        std::cerr << "BCryptCreateHash failed: 0x" << std::hex << status << std::dec << std::endl;
    ...
    // 6. Добавить данные для хэширования
    status = BCryptHashData(hHash, (PBYTE)input.c_str(), (ULONG)input.size(), 0);
    if (status != 0) {
        std::cerr << "BCryptHashData failed: 0x" << std::hex << status << std::dec << std::endl;
    ...
    // 7. Финализировать хэш и получить результат
    status = BCryptFinishHash(hHash, pbHash, cbHash, 0);
    if (status != 0) {
        std::cerr << "BCryptFinishHash failed: 0x" << std::hex << status << std::dec << std::endl;
        BCryptDestroyHash(hHash);
        HeapFree(GetProcessHeap(), 0, pbHashObject);
        HeapFree(GetProcessHeap(), 0, pbHash);
        BCryptCloseAlgorithmProvider(hAlgorithm, 0);
    ...
    // 8. Преобразовать в шестнадцатеричную строку
    char hex[3];
    for (DWORD i = 0; i < cbHash; i++) {
        sprintf_s(hex, sizeof(hex), "%02x", pbHash[i]);
        result += hex;
    }
