...
public:
    ...
    ~Timer() {
        spdlog::info("{} took {} ms", name, elapsed_ms());
    }
};

/*--------------------------------------------------------------------
Initialization of the SPDLogger.
----------------------------------------------------------------------*/
int InitSPDLogger()
{
    try 
    {
        ...
        auto logger = std::make_shared<spdlog::logger>("word_freq", sinks.begin(), sinks.end());
...
/*-----------------------------------------------------------------------------
 Function: splitWordsForThreadsBalanced() 
 -----------------------------------------------------------------------------
 divides a sequence of words (indexed 0 to totalWords-1) into balanced blocks 
 for parallel processing. It returns a vector of ThreadBlock structs, each 
 containing a start and end index (inclusive). 
...
{
    Timer timer("writeResultsToFile");
    try 
    {
...
int main(int argc, char* argv[]) 
{
    ...
    try
    {
        ...
        4. Sort by frequency
        --------------------------------------------------------------------------*/
        std::vector<std::pair<std::string, size_t>> sortedWords(wordFreq.frequencies.begin(), 
            wordFreq.frequencies.end());
